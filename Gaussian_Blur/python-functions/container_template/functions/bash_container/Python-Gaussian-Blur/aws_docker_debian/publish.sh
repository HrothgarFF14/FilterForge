...
aws ecr create-repository --repository-name python-gaussian-arm64 --image-scanning-configuration scanOnPush=true

echo "Pushing Docker Image..."
registryID=$(aws ecr describe-registry | jq '.registryId' | tr -d '"')
docker tag ${function}:latest ${registryID}.dkr.ecr.${region}.amazonaws.com/python-gaussian-arm64:${function}
aws ecr get-login-password | docker login --username AWS --password-stdin ${registryID}.dkr.ecr.${region}.amazonaws.com
docker push ${registryID}.dkr.ecr.${region}.amazonaws.com/python-gaussian-arm64:${function}
docker logout

echo "Deploying function..."
code={\"ImageUri\":\"${registryID}.dkr.ecr.${region}.amazonaws.com/python-gaussian-arm64:${function}\"}
...
if [ 0 -eq $? ]; then
	...
	aws lambda update-function-code \
		--function-name $function \
		--image-uri ${registryID}.dkr.ecr.${region}.amazonaws.com/python-gaussian-arm64:${function} 
	aws lambda wait function-updated --function-name "$function"
