...
def resizeFunction(request, context):
    inspector = Inspector()
    ...
    try:
        ...
        new_width = int(request.get('newWidth'))
        new_height = int(request.get('newHeight'))
        ...
        logger.info(f"Downloading image {filename} from bucket {input_bucket}")
        ...
        logger.info(f"Uploading resized image to bucket {output_bucket}")
        upload_image_to_s3(s3_client, resized_image, output_bucket, output_filename, "png")

        inspector.addAttribute("message", f"Image resized and stored at s3://{output_bucket}/{output_filename}")
