...
width=100
height=100
outputFilename="large_image-crop.png"
...
time output=$(aws lambda invoke --invocation-type RequestResponse --cli-binary-format raw-in-base64-out --function-name filterforge-java-crop-x86 --region us-east-1 --payload "$json" /dev/stdout | head -n 1 | head -c -2 ; echo)
