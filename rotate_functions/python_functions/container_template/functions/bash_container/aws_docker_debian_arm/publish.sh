...
aws ecr create-repository --repository-name saaf-functions-arm --image-scanning-configuration scanOnPush=true

echo "Pushing Docker Image..."
registryID=$(aws ecr describe-registry | jq '.registryId' | tr -d '"')
docker tag ${function}:latest ${registryID}.dkr.ecr.${region}.amazonaws.com/saaf-functions-arm:${function}
aws ecr get-login-password | docker login --username AWS --password-stdin ${registryID}.dkr.ecr.${region}.amazonaws.com
docker push ${registryID}.dkr.ecr.${region}.amazonaws.com/saaf-functions-arm:${function}
docker logout

echo "Deploying function..."
code={\"ImageUri\":\"${registryID}.dkr.ecr.${region}.amazonaws.com/saaf-functions-arm:${function}\"}
...
if [ 0 -eq $? ]; then
	...
	aws lambda update-function-code \
		--function-name $function \
		--image-uri ${registryID}.dkr.ecr.${region}.amazonaws.com/saaf-functions-arm:${function} 
	aws lambda wait function-updated --function-name "$function"
